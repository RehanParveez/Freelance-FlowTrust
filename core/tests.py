...
from payments.models import Wallet, Escrow, Payment, PaymentMethod, Refund, Transaction
...
class OwnerOrAdminPermissionTests(TestCase):
  def setUp(self):
    self.admin_user = User.objects.create_user(username = 'admin', email = 'admin@gmail.com', control = 'admin', password = 'pass')
    self.client_user = User.objects.create_user(username = 'client', email = 'client@gmail.com', control = 'client', password = 'cli123456')
    self.freelancer_user = User.objects.create_user(username = 'freelancer', email = 'freelancer@gmail.com', control = 'freelancer', password = 'free123456')
    self.freelancer_profile = FreelancerProfile.objects.create(user=self.freelancer_user)
    self.contract = Contract.objects.create(client=self.client_user, freelancer=self.freelancer_user, title = 'contract test', total_amount=1000)
    self.participant = ContrParticipant.objects.create(contract=self.contract, user=self.freelancer_user, role = 'freelancer')
    self.term = ContractTerm.objects.create(contract=self.contract, description = 'term test')
    ...
    self.activity = Activity.objects.create(user=self.client_user, action_type = 'created contract', content_object=self.contract)
    self.milestone = Milestone.objects.create(contract=self.contract, title = 'milestone 1', amount=600)
    self.submission = MilestoneSubmission.objects.create(milestone=self.milestone, submitted_by=self.freelancer_user, notes = 'this is milestone sub')
    ...
    self.dispute = Dispute.objects.create(contract=self.contract, raised_by=self.client_user, description = 'dispute description')
    self.dispute_message = DisputeMessage.objects.create(dispute=self.dispute, by=self.client_user, content = 'message content')
    self.proof = Proof.objects.create(dispute=self.dispute, file = 'proof file')
    self.wallet = Wallet.objects.create(user=self.client_user)
    ...
    self.transaction = Transaction.objects.create(wallet=self.wallet, amount=700)
    self.refund = Refund.objects.create(payment=self.payment, amount=300, reason = 'the reason')
    self.payment_method = PaymentMethod.objects.create(user=self.client_user, method_type = 'card')
    self.job = JobPost.objects.create(client=self.client_user, name = 'job test', budget=600)
    self.proposal = Proposal.objects.create(job=self.job, freelancer=self.freelancer_profile, proposed_amount=450)
    ...
    self.notification = Notification.objects.create(user=self.client_user, content='notification content')
    self.talk = Talk.objects.create(contract=self.contract)
  ...
  def all_perm(self, user):
    request = self.factory.get('/practice-url/')
    request.user = user
    all_objects = [self.contract, self.participant, self.term, self.contract_status, self.activity, self.milestone, self.submission, self.review, self.milestone_status, self.dispute, self.dispute_message, self.proof,
    self.wallet, self.escrow, self.payment, self.transaction, self.refund, self.payment_method, self.job, self.proposal, self.profile, self.ver_status,
    self.review_model, self.rating, self.user_analytics, self.earning_report, self.contract_analytics, self.notification, self.talk, self.message]
